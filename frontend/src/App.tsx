...
export default function App() {
  ...
  // user の未知のフィールドに対しては any で安全にアクセス
  const displayName = (user as any)?.user_display_name ?? (user as any)?.display_name ?? (user as any)?.user_email ?? "ログイン中のユーザー";
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
            ...
            <button
              ...
              className={`px-4 py-2 rounded ${
                currentView === "report" ? "bg-blue-600 text-white" : "bg-gray-200"
              }`}
            >
              報告投稿
            </button>
            <div className="text-sm text-gray-600">{displayName}</div>
            <button onClick={() => logout && logout()} className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">
              ログアウト
            </button>
          </div>
        </div>
        ...
        {currentView === "dashboard" ? (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm opacity-90">訓練の実施報告を簡単操作で送信できます。写真や位置情報も添付可能です。</p>
                  </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setCurrentView("dashboard")} className="px-3 py-1 text-sm bg-gray-200 rounded">
                戻る
              </button>
            </div>
            <ReportForm
